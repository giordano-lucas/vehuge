...
if __name__ == '__main__':
    ...
    min_sample_leaves = [1000, 300, 100, 30, 10, 1]
    min_sample_leaves = [msl for msl in min_sample_leaves if msl < data.shape[0]/10]

    filepath = os.path.join('missing', FLAGS.dataset + str(FLAGS.runs[0]) + '_test.csv')
    ...
    for run in FLAGS.runs:
        ...
        np.savetxt(os.path.join(dir, 'folds.txt'), folds, fmt='%d')  # We still save them just in case
        ...
        for min_samples_leaf in min_sample_leaves:
            ...
            for fold in range(FLAGS.n_folds):
                ...
                print('            Training')
                if FLAGS.sklearn:
                    rf = SKRandomForest(n_estimators=FLAGS.n_estimators, ncat=ncat, min_samples_leaf=min_samples_leaf)
                    rf.fit(X_train, y_train)
                else:
                    rf = RandomForest(n_estimators=FLAGS.n_estimators, ncat=ncat, min_samples_leaf=min_samples_leaf, surrogate=FLAGS.surrogate)
                    rf.fit(X_train, y_train)
